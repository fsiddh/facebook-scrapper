...
class Facebook:
    ...
    def extract_comments(self, session, base_url, post_bs, post_url):
        ...
        while True:
            ...
            if first_comment_page:
                ...
                post_bs = self.get_bs(session, base_url+show_more_url)
                time.sleep(3)
    ...
    def scrape_post(self, session, base_url, post_url):

        # Goes to post URL and extracts post data.
        post_data = OrderedDict()

        post_bs = self.get_bs(session, base_url+post_url)
        time.sleep(5)
        ...
        try:
            post_data['comments'] = self.extract_comments(session, base_url, post_bs, post_url)
        except Exception:
            post_data['comments'] = []
    ...
    def crawl_profile(self, session, base_url, profile_url, post_limit):
        ...
        profile_bs = self.get_bs(session, profile_url)
        n_scraped_posts = 0
        scraped_posts = list()
        posts_id = None
        ...
        while n_scraped_posts < post_limit:
            ...
            for post_url in posts_urls:
                # print(post_url)
                try:
                    post_data = self.scrape_post(session, base_url, post_url)
                    scraped_posts.append(post_data)
                except Exception as e:
                    logging.info('Error: {}'.format(e))
                n_scraped_posts += 1
                if self.posts_completed(scraped_posts, post_limit):
                    break
            
            show_more_posts_url = None
            if not self.posts_completed(scraped_posts, post_limit):
                show_more_posts_url = profile_bs.find('div', id=posts_id).next_sibling.a['href']
                profile_bs = self.get_bs(session, base_url+show_more_posts_url)
                time.sleep(3)
            else:
                break
